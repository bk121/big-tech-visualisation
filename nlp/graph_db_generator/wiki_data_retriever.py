...
json_path = os.path.join(dirname, 'head_company.json')
...
class WikiDataRetriever:
    ...
    def get_property_values(self, property_code):
        ...
        for (i, code) in enumerate(entity_codes):
            ...
            try:
                ...
                continue
        try:
            os.remove(os.path.join(dirname, "temp.json"))
    ...
    def __download_entity_json(self, code, filename):
        urllib.request.urlretrieve(
            ...
            os.path.join(dirname, filename + ".json"),
        )
        return

    def __read_json(self, filename):
        with open(os.path.join(dirname, filename + ".json")) as data_file:
    ...
    def __retrieve_location(self, code, data):
        try:
            ...
            os.remove(os.path.join(dirname, "loc.json"))
